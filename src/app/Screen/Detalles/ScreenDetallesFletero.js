...
export default function ScreenDetallesFletero() {
  ...
  useEffect(() => {
    async function cargarDetalle() {
      ...
      setSolicitud(data);
      setOrigen(data.punto_ruta?.find((p) => p.tipo === 'origen')?.direccion_texto ?? 'No definido');
      setDestino(data.punto_ruta?.find((p) => p.tipo === 'destino')?.direccion_texto ?? 'No definido');

      if (data.estado === 'aceptada' || data.estado === 'en_progreso' || data.estado === 'completada') {
        setYaTomada(true);
      }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.infoGrid}>
          ...
          <View style={styles.infoItem}>
            <Text style={styles.infoLabel}>Oferta del cliente</Text>
            <Text style={[styles.infoValor, styles.precioDestacado]}>
              {precioMostrar ? `$${precioMostrar}` : 'Pendiente'}
            </Text>
          </View>
        </View>
      </View>
      ...
      >
        {aceptando ? (
          ...
          <Text style={styles.textoBotonAceptar}>
            {yaTomada ? 'Ya no disponible' : `Aceptar por $${precioMostrar}`}
          </Text>
        )}
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  fotoCargamento: { width: 110, height: 110, borderRadius: 12, marginRight: 10, backgroundColor: '#E2E8F0' },
  avisoTomada: {
