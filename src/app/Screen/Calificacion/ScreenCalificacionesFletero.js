...
const CONFIG = {
  ...
  campoFleteroId: 'fletero_id',    // FK hacia el fletero
  campoCalificadorId: 'calificador_id', // FK hacia el usuario que califica
...
export default function ScreenCalificacionesFletero() {
  const router = useRouter();
  const { user } = useAuth(); // usuario autenticado (quien califica)

  // Datos que llegan por navegación al abrir esta pantalla, ej:
  // router.push({ pathname: '/Screen/Calificacion/ScreenCalificacionesFletero', params: { fleteroId, fleteroNombre, servicioId } })
  const { fleteroId, fleteroNombre, servicioId } = useLocalSearchParams();

  const [rating, setRating] = useState(0);
  const [comentario, setComentario] = useState('');
  const [enviando, setEnviando] = useState(false);

  const iniciales = (fleteroNombre || '')
    .split(' ')
    .map((p) => p[0])
    .join('')
    .slice(0, 2)
  ...
  const handleEnviar = async () => {
    ...
    try {
      const payload = {
        [CONFIG.campoFleteroId]: fleteroId,
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{iniciales || 'CM'}</Text>
        </View>

        <Text style={styles.nombre}>{fleteroNombre || 'Fletero'}</Text>
        <Text style={styles.pregunta}>¿Cómo estuvo el servicio?</Text>
